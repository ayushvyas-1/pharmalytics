...
export default function AnalyticsPage() {
  ...
  const [selectedPresentation, setSelectedPresentation] = useState<string | null>("all")

  // Get data from our database
  const doctors = getDoctors()
  const presentations = getPresentations()
  const recentSessions = getRecentSessions(50) // Get more sessions for analytics
  ...
  const totalTimeSpent = allSlideAnalytics.reduce((sum, analytic) => sum + analytic.timeSpent, 0) / 1000 // Convert to seconds

  console.log("Calculated stats:", { totalSessions, activeDoctors, totalTimeSpent })
  ...
  const getFilteredSessions = () => {
    ...
    if (selectedPresentation !== "all") {
      sessions = sessions.filter((s) => s.presentation.id === selectedPresentation)
    }
  ...
  const getSlideAnalyticsForPresentation = (presentationId: string) => {
    ...
    sessions.forEach((session) => {
      const analytics = allSlideAnalytics.filter((a) => a.sessionId === session.id)
  ...
  const slideAnalytics =
    selectedPresentation !== "all" ? getSlideAnalyticsForPresentation(selectedPresentation) : topSlides
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Card className="bg-white shadow-sm">
          ...
          <CardContent>
            ...
            <Select value={selectedDoctor || "all"} onValueChange={(value) => setSelectedDoctor(value || "all")}>
        ...
        <Card className="bg-white shadow-sm">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Time spent on slides</p>
            <Select
                ...
                filteredSessions.slice(0, 8).map((session, index) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{session.presentation.title}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium">{formatTime(session.duration)}</p>
                      <p className="text-xs text-muted-foreground">
                        {session.session.endTime ? getTimeAgo(session.session.endTime) : "In progress"}
                      </p>
                    </div>
                  </div>
                ))
                  ...
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {presentations.find((p) => p.id === item.slide.presentationId)?.title}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium">{formatTime(item.avgTimeSpent / 1000)}</p>
                      <p className="text-xs text-muted-foreground">{item.views} views</p>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-muted-foreground text-center py-4">No slide analytics yet</p>
              )}
            </div>
          </CardContent>
        </Card>
      ...
      {/* Rest of the tabs content remains the same but will show real data */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      {filteredSessions.length > 0 ? (
                        filteredSessions.slice(0, 5).map((session, index) => (
                          <tr key={index} className="border-b hover:bg-gray-50">
                            <td className="py-3 px-4 font-medium">{session.doctor.name}</td>
                            <td className="py-3 px-4">{session.presentation.title}</td>
            ...
            <Card className="bg-white shadow-sm">
              <CardHeader>
                ...
                <CardDescription>Slides with highest engagement time</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <CardHeader>
              ...
              <CardDescription>
                ...
                {selectedPresentation !== "all" &&
                  ` for ${presentations.find((p) => p.id === selectedPresentation)?.title}`}
              </CardDescription>
            </CardHeader>
            <CardContent className="px-0 sm:px-6">
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {slideAnalytics.length > 0 ? (
                      slideAnalytics.map((item, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          <td className="py-3 px-4 font-medium">{item.slide.title}</td>
                          <td className="py-3 px-4">
                            {presentations.find((p) => p.id === item.slide.presentationId)?.title}
                          </td>
                          <td className="py-3 px-4">{item.views}</td>
                          <td className="py-3 px-4">{formatTime(item.avgTimeSpent / 1000)}</td>
                          <td className="py-3 px-4">{formatTime(item.totalTimeSpent / 1000)}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan={5} className="py-8 text-center text-muted-foreground">
                          No slide analytics available yet
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        <TabsContent value="sessions">
          <Card className="bg-white shadow-sm">
            <CardHeader>
              ...
              <CardDescription>Complete history of presentation sessions</CardDescription>
            </CardHeader>
            <CardContent className="px-0 sm:px-6">
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredSessions.length > 0 ? (
                      filteredSessions.map((session, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          <td className="py-3 px-4 font-medium">{session.doctor.name}</td>
                          <td className="py-3 px-4">{session.doctor.specialty}</td>
                          <td className="py-3 px-4">{session.presentation.title}</td>
                          <td className="py-3 px-4">
                            {session.session.endTime ? getTimeAgo(session.session.endTime) : "In progress"}
                          </td>
